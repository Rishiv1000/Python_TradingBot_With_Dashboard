...
def main():
    print("Starting Kite Automated Trading Bot (RSI)...")
    kite = generate_or_load_session(allow_interact=False)
    paper_trade = config.PAPER_TRADE
    
    from engine_entry import EntryEngineRSI
    from engine_exit import ExitEngineRSI
    entry_engine_class = EntryEngineRSI
    exit_engine_class = ExitEngineRSI
    ...
    while True:
        if config.BOT_RUNNING:
            ...
            for i in range(int(wait_time), 0, -1):
                ...
                msg = f"[RSI] [{timeframe}] Waiting... {i}s"
