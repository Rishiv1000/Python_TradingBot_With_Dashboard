...
from order_manager import place_real_sell
import threading
import time
import sqlite3
...
class ExitEngineGreen:
    ...
    def _fetch_open_positions(self):
        ...
        rows = [dict(zip(colnames, r)) for r in res]
        conn.close()
        return rows
    ...
    def _close_position_and_log(self, position, sell_price, reason, sell_order_id):
        ...
        conn = self._db_connection(); cursor = conn.cursor()
        cursor.execute("INSERT INTO trades_log (symbol, buytime, buyprice, selltime, sellprice, pnl, reason, slippage, buy_order_id, sell_order_id, strategy) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (position["symbol"], position["buytime"], position["buyprice"], datetime.now(), sell_price, pnl, reason, slippage_amt, str(position["buy_order_id"]), str(sell_order_id), position.get("strategy")))
        cursor.execute("UPDATE symbols_state SET isExecuted=0, buyprice=NULL, buytime=NULL, buy_order_id=NULL, product=NULL, strategy=NULL, last_sell_time=? WHERE symbol=?", (datetime.now(), position["symbol"]))
    ...
    def place_real_sell(self, row):
        try:
            ...
            return place_real_sell(self.kite, symbol, qty, exchange, product, tag=orig_order_id)
    ...
    def _perform_sell(self, row, ltp, reason):
        ...
        sell_price = ltp
        # If real order, try to get fill price
