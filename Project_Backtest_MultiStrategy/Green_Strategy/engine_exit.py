...
class ExitEngineGreen:
    ...
    def _fetch_open_positions(self):
        ...
        # No strategy filter needed as it is a dedicated table
        cursor.execute("SELECT * FROM symbols_green WHERE isExecuted=1")
    ...
    def _close_position_and_log(self, position, sell_price, reason, sell_order_id):
        ...
        """, (
            ...
            sell_price, pnl, reason, slippage_amt, str(position["buy_order_id"]), 
            str(sell_order_id), position["mode"], "GREEN"
        ))
        
        cursor.execute("""
            UPDATE symbols_green 
        ...
        conn.commit()
        conn.close()
        print(f"✅ [GREEN] SOLD {position['symbol']} at {sell_price} ({reason})")
    ...
    def start_monitoring(self):
        ...
        print("🛡️ [GREEN] Exit Monitor active.")
        while True:
