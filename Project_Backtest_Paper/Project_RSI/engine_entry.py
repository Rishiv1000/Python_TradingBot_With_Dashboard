...
class EntryEngineRSI:
    ...
    def fake_place_order(self, symbol, quantity, exchange):
        print(f"🚫 SIMULATED REAL BUY (RSI): You are on BackTest Module. Real Order Blocked for {symbol}.")
        return f"RSI-FORCED-REAL-BUY-{symbol}-{int(time.time())}"
    ...
    def run_cycle(self):
        ...
        for item in symbol_data_list:
            ...
            try:
                ...
                records = fetch_symbol_candles(self.kite, token, timeframe=getattr(config, 'TIMEFRAME', 'minute'), days=lookback)
